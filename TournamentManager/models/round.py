...
class Round:
    ...
    def create_pairs_based_on_points(self):
        ...
        while len(available_players) >= 2:
            ...
            # Get a list of players with the same or close points (up to one group lower if needed)
            possible_opponents = self.get_possible_opponents(player1, grouped_players, available_players)
            ...
            if player2:
                ...
                raise Exception("No valid opponent found, which shouldn't happen with this logic.")
    ...
    def get_possible_opponents(self, player1, grouped_players, available_players):
        ...
        possible_opponents = [player for player in possible_opponents if player in available_players]
        ...
        while len(possible_opponents) < 2:
            next_lower_points = self.get_next_lower_points(grouped_players, current_points)
            if next_lower_points is None:
                break  # No more lower groups available
            possible_opponents.extend(
                [player for player in grouped_players[next_lower_points] if player in available_players]
            )
    ...
    def find_best_opponent(self, player1, possible_opponents):
        ...
        for player2 in possible_opponents:
            if not player1.has_played_against(player2):
                return player2
        # If all possible opponents have played against player1, return the one with the fewest encounters
        min_encounters = min(player1.opponents.count(player2) for player2 in possible_opponents)
